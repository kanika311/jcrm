...
export default function CoursesCatalogClient({
  ...
  // Reusable Filter Sidebar Content (Desktop + Mobile)
  const FilterSidebarContent = (
    <div className="flex h-full flex-col bg-white dark:bg-gray-900">
      {/* Sidebar Header (Dark Navy with orange border accent like yogsathi) */}
      <div className="shrink-0 flex items-center justify-between border-b-4 border-orange-500 bg-[#0a2333] px-4 py-3 text-white">
        <div className="flex items-center gap-2.5">
          <span className="rounded-lg bg-orange-500 p-1.5 shadow-xs">
            <FiSliders className="h-4 w-4 text-white" />
          </span>
          <span className="text-xs font-black tracking-wider uppercase">FIND YOUR COURSE</span>
        </div>
        {mobileFilterOpen && (
          <button
            type="button"
      ...
      <div className="flex-1 overflow-y-auto overscroll-contain px-4 py-2 text-xs divide-y divide-orange-100/70 dark:divide-gray-800">
        
        {/* Course Level */}
        <div className="py-3.5">
          <h4 className="mb-2 flex items-center gap-2 font-bold uppercase tracking-wider text-[#0a2333] dark:text-orange-300">
          ...
          <select
            ...
            className="w-full px-3 py-2 rounded-xl text-xs font-semibold bg-slate-50 dark:bg-gray-800 border border-orange-200/80 dark:border-gray-700 text-slate-700 dark:text-slate-200 focus:outline-none focus:border-orange-500 focus:ring-2 focus:ring-orange-500/20"
          >
        ...
        <div className="py-3.5">
          <div className="flex items-center justify-between mb-2">
            <h4 className="flex items-center gap-2 font-bold uppercase tracking-wider text-[#0a2333] dark:text-orange-300">
              <span className="h-2 w-2 rounded-full bg-orange-500" />
              MAX TUITION
            </h4>
      ...
      <div className="shrink-0 border-t-4 border-orange-500 bg-[#0a2333] p-3 text-white">
        <div className="flex gap-2">
          <button
            type="button"
            onClick={handleResetFilters}
            className="flex-1 py-2 rounded-xl border border-gray-600 bg-white/5 hover:bg-white/10 text-white font-bold text-xs transition"
          >
            Clear All
          </button>
          {mobileFilterOpen && (
            <button
              type="button"
              onClick={() => setMobileFilterOpen(false)}
              className="flex-1 py-2 rounded-xl bg-orange-500 hover:bg-orange-600 text-white font-bold text-xs transition"
            >
    ...
    <div className="min-h-screen pt-24 pb-20 bg-[#f8fafc] dark:bg-gray-950">
      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8">
        <div className="flex flex-col lg:flex-row items-start gap-6">
          ...
          {/* DESKTOP LEFT FILTER SIDEBAR (Sticky on scroll) */}
          <aside className="hidden lg:block w-[280px] shrink-0 sticky top-24 h-[calc(100vh-7.5rem)] rounded-2xl border border-[#0a2333]/20 bg-white shadow-md ring-1 ring-orange-500/10 dark:border-orange-500/40 dark:bg-gray-900 overflow-hidden">
          ...
          {/* RIGHT COLUMN: STICKY SEARCH BAR + COURSES GRID */}
          <main className="flex-1 min-w-0 w-full">

            {/* STICKY SEARCH BAR (like yogsathi.com/members) */}
              ...
              <div className="flex items-center gap-2 sm:gap-3">
                ...
                <button
                  ...
                  className={`relative flex h-[50px] shrink-0 items-center justify-center gap-2 rounded-2xl border-2 px-3.5 text-xs font-bold shadow-xs transition active:scale-95 lg:hidden ${
                    activeFilterCount > 0
                      ? "border-orange-500 bg-[#0a2333] text-white ring-2 ring-orange-500/15"
                      : "border-orange-500 bg-white text-slate-800 ring-2 ring-orange-500/15 dark:border-orange-500 dark:bg-gray-900 dark:text-white"
                  }`}
                  aria-label="Open filters"
                >
                  <FiSliders className={`h-4 w-4 ${activeFilterCount > 0 ? "text-orange-400" : "text-orange-500"}`} />
                  <span className="hidden sm:inline">Filters</span>
                  {activeFilterCount > 0 && (
                    <span className="flex h-4 min-w-4 items-center justify-center rounded-full bg-orange-500 px-1 text-[10px] font-bold text-white shadow-xs">
                      {activeFilterCount}
                    </span>
                  )}
                </button>

                {/* Main Prominent Sticky Search Box */}
                <div className="relative flex flex-1 items-center rounded-2xl border-2 border-orange-500 bg-white shadow-sm ring-2 ring-orange-500/15 dark:border-orange-500 dark:bg-gray-900">
                  <FiSearch className="pointer-events-none absolute left-4 top-1/2 h-4 w-4 -translate-y-1/2 text-orange-500" />
                  <input
                    type="text"
                    value={searchQuery}
              ...
              <div className="flex items-center justify-between mt-2.5 px-1 text-xs">
                ...
                {activeFilterCount > 0 && (
                  <button
                    type="button"
                    onClick={handleResetFilters}
                    className="text-orange-600 hover:text-orange-700 dark:text-orange-400 font-bold transition flex items-center gap-1"
                  >
            ...
            {filteredCourses.length === 0 ? (
              <div className="text-center py-20 bg-white dark:bg-gray-900 rounded-2xl border border-dashed border-orange-200 dark:border-gray-800 p-8 shadow-sm">
                ...
                <button
                  type="button"
                  onClick={handleResetFilters}
                  className="px-4 py-2 rounded-xl text-xs font-bold text-white bg-orange-500 hover:bg-orange-600 transition shadow-sm"
                >
                  Clear All Filters
                </button>
              </div>
